...
module Cypress
  ...
  class PatientZipper
    ...
    def self.zip(file, patients, format, measure, start_date, end_date)
      # patients = apply_sort_to patients
      # mes, sd, ed = mes_start_end(patients)

      formatter = if format.to_sym == :qrda
                    Cypress::QRDAExporter.new(measure, start_date, end_date)
                  else
                    Cypress::HTMLExporter.new(measure, start_date, end_date)
                  end
