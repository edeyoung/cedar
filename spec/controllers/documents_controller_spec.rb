...
RSpec.describe API::V1::DocumentsController, type: 'controller' do
  ...
  it 'index gets documents in order' do
    get :index, params: { test_execution_id: @te.id }
  ...
  it 'show gets single document' do
    get :show, params: { test_execution_id: @te.id, id: 0 }
    expect(response).to have_http_status(:success)
    assert_not_nil json(response)['data']['attributes']['qrda']
  end

  it 'report correct result' do
    put :update, params: { test_execution_id: @te.id, id: 0, actual_result: @te.documents[0].expected_result }
    expect(response).to have_http_status(:success)
    assert_equal 'passed', json(response)['data']['attributes']['state']
  end

  it 'bulk report' do
    patch :report_results, params: { test_execution_id: @te.id,
                                     results: {
